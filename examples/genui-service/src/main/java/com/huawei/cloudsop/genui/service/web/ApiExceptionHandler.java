...
public class ApiExceptionHandler {
    ...
    /**
     * SDK 只抛一种异常类型;名称碰撞类消息以固定前缀开头(见 GenerationSdk)。 必须锚定前缀而非子串匹配:其余消息会拼接用户提供的名称, 名称本身含 "collision" 时子串匹配会把 400 误判成 409。
     */
    ...
    public ResponseEntity<ErrorResponse> handleSdk(GenerationSdkException ex) {
        ...
        HttpStatus status = message.startsWith("Component name collision") || message.startsWith("Tool name collision")
                ? HttpStatus.CONFLICT
