 ...
 *
 * <p>
 * 错误行为对齐被替换的 Node 版:流开始前失败 → 502 JSON(异常处理器); 流中途失败或 finish_reason 非 stop → 流尾追加 "[ERROR: ...]" 后正常关闭。
 *
 * @since 2026
 */
@RestController
public class GenerateController {
    ...
    public ResponseEntity<StreamingResponseBody> generate(@RequestBody GenerateRequest body) {
        ...
        StreamingResponseBody responseBody = out -> {
            ...
            if (piped && !"stop".equals(finishReason)) {
                ...
                writeTail(out, "[ERROR: incomplete, finish_reason="
    ...
    private String resolveSystemPrompt(GenerateRequest body) {
        ...
        GenUIPromptRequest request = new GenUIPromptRequest(body.getExtensionId(), dataModel, body.getExtraRules(),
                null, null, null, null);
