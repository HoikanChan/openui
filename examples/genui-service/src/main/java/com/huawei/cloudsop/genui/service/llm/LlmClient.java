...
public class LlmClient {
    ...
    public LlmClient(LlmProperties props) {
        ...
        if (props.apiKey() == null || props.apiKey().isBlank()) {
            log.warn(
                    "[llm] LLM_API_KEY is not set — /generate will fail with 502; registration/assembly endpoints still work");
        }
    }
    ...
    public LlmStream openStream(String systemPrompt, String userPrompt) {
        ...
        HttpRequest request = HttpRequest.newBuilder().uri(URI.create(chatCompletionsUrl(props.baseUrl())))
                .header("Authorization", "Bearer " + props.apiKey()).header("Content-Type", "application/json")
                .header("Accept", "text/event-stream").POST(HttpRequest.BodyPublishers
                        .ofString(buildRequestBody(systemPrompt, userPrompt), StandardCharsets.UTF_8))
                .build();
        try {
