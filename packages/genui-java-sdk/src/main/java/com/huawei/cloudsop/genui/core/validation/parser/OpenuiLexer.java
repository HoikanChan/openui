...
/**
 * Hand-written lexer for openui-lang.
 *
 * <p>Mirrors {@code packages/lang-core/src/parser/lexer.ts} tokenization rules exactly (string
 * escapes, number formats, {@code $state}, {@code @builtin}, PascalCase {@link TokenType#TYPE} vs
 * lowercase {@link TokenType#IDENT}, two-char operators {@code == != >= <= && || ??}).
 *
 * <p>Java extension: every {@link Token} carries 1-based line/column and 0-based offset/length so
 * downstream stages can attach diagnostics. Newlines are significant tokens (statement separators).
 */
public final class OpenuiLexer {

  private OpenuiLexer() {}

  /** Tokenize a source string into a flat token list terminated by an {@link TokenType#EOF}. */
  public static List<Token> tokenize(String src) {
    ...
    while (i < n) {
      ...
      if (c == '$' && i + 1 < n && isIdentStart(src.charAt(i + 1))) {
        ...
        tokens.add(
            Token.ofText(
                TokenType.STATE_VAR, src.substring(start, i), startLine, startCol, startOff, len));
        col += len;
        continue;
      }
  ...
  /**
   * Decode a double-quoted raw literal (including surrounding quotes) into its runtime string,
   * mirroring the TS path that delegates to {@code JSON.parse}. On malformed escapes it falls back
   * to stripping the surrounding quotes (parity with the TS catch branch).
   */
