...
The demo server SHALL build its system prompt from the `dslLibrary` exported by `@cloudsop/openui-react-ui-dsl` so prompt generation and client rendering use the same DSL definition.
...
The demo documentation SHALL describe the dependency, build, and runtime prerequisites required to run the example against `@cloudsop/openui-react-ui-dsl` inside the workspace.
