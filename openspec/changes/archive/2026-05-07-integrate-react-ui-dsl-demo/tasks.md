...
- [x] 1.1 Add `@cloudsop/openui-react-ui-dsl` and any required peer/runtime dependencies to `examples/react-ui-dsl-demo/package.json`
- [x] 1.2 Decide and implement the supported local resolution path for consuming `@cloudsop/openui-react-ui-dsl` from the demo client and server
...
- [x] 2.1 Replace the demo renderer import so `src/App.tsx` uses `dslLibrary` from `@cloudsop/openui-react-ui-dsl`
...
- [x] 3.3 Verify the demo client renders generated DSL with `@cloudsop/openui-react-ui-dsl` in the monorepo development flow
