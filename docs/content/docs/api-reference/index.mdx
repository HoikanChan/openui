---
title: OpenUI SDK
description: API reference for the documented surfaces of @cloudsop/openui-react-lang, @openuidev/react-headless, @openuidev/react-ui, and @openuidev/react-email.
---

The OpenUI SDK is split into packages that build on each other:

- **`@cloudsop/openui-react-lang`** — Core runtime. Define component libraries with Zod schemas, generate system prompts, parse OpenUI Lang, and render streamed output to React. This is the foundation — you need it for any OpenUI integration.
...
- **`genui-java-sdk` (Java)** — Back-end Java SDK for UI-generation services: register Generation Context extensions and assemble system prompts byte-aligned with the front-end library. Ships with a REST reference implementation (`examples/genui-service`).

## Packages

<Cards>
  <Card title="@cloudsop/openui-react-lang" href="/docs/api-reference/react-lang">
  ...
  <Card title="genui-java-sdk (Java)" href="/docs/api-reference/genui-java-sdk">
    GenerationSdk for back-end services: Generation Context registration, Request Overlay, and
    prompt assembly byte-aligned with the front-end library, plus the GenUI Service REST reference.
  </Card>
</Cards>
