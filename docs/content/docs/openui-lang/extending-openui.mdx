...
写出的 `billing-extension.json` 就是一个**可直接注册的 `GenUIExtension` 形 JSON**：顶层带 `extensionId` / `version`，`components` 用 `propsSchema`（JSON Schema）描述，且只含你扩展的组件，不含 base 内置件：
...
后端拿到这段 JSON，反序列化成 `GenUIExtension` 直接注册（见下方「直接嵌入 Java SDK」）。因为前后端出自同一个库定义，`propsSchema.properties` 的字段顺序和前端 Zod props 顺序天然对齐，不用人工核对。
...
如果你直接嵌入 Java Generation SDK，注册入口只有一个：`sdk.register(GenUIExtension)`。扩展契约就是上面 `generate-component-spec` 导出的那段 JSON（也可以来自配置中心、文件或数据库），用你自己的 JSON 库反序列化成 `GenUIExtension` 后直接注册，不必在 Java 里手搓嵌套 `Map`：
...
GenUIExtension generation = new ObjectMapper().readValue(extensionJson, GenUIExtension.class);
