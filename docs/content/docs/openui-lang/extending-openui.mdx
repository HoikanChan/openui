...
| 业务规则 | 在组件描述、`componentGroups.notes`、`PromptOptions.additionalRules` 中表达 | 在 Generation Extension 或 Request Overlay 的 `additionalRules` / `extraRules` 中表达 |
...
export const billingLibrary = openuiLibrary.extend({
...
如果你使用 `@openuidev/react-ui` 的 chat 布局，同样传 `componentLibrary`：

```tsx
<FullScreen
  processMessage={processMessage}
  streamProtocol={openAIAdapter()}
  componentLibrary={billingLibrary}
  agentName="Assistant"
/>
```
...
后端契约不用手写——前端用 `generateComponentSpecs(...)` 从组件定义直接生成 renderer-free 的组件契约（只含 `description` + `propsSchema`，不含 React 实现），再包成扩展对象写出 JSON。组件名和 `propsSchema.properties` 顺序天然和前端 Zod props 一致：

```ts
import { writeFileSync } from "node:fs";
import { generateComponentSpecs } from "@openuidev/react-lang";
import { BillingCard } from "./billing-library"; // 记得 export 你定义的组件

// 只传你扩展的组件；返回 { [name]: { description, propsSchema } }
const components = generateComponentSpecs([BillingCard]);
...
writeFileSync("billing-extension.json", `${JSON.stringify(extension, null, 2)}\n`);
```

> `generateComponentSpecs` 要求每个组件的必填 props 排在可选 props 之前（对应 openui-lang 的位置参数顺序），否则会抛错。

写出的 `billing-extension.json` 就是一个**可直接注册的 `GenUIGeneration` 形 JSON**：顶层带 `extensionId` / `version`，`components` 用 `propsSchema`（JSON Schema）描述，且只含你扩展的组件，不含 base 内置件：
...
后端拿到这段 JSON，反序列化成 `GenUIGeneration` 直接注册（见下方「直接嵌入 Java SDK」）。因为前后端出自同一个库定义，`propsSchema.properties` 的字段顺序和前端 Zod props 顺序天然对齐，不用人工核对。

### 通过 REST 注册

当前 GenUI Service 的 REST 参考实现使用 `/v1/generations` 管理扩展。`extensionId` 用来隔离不同业务扩展，重复注册同一个 `extensionId` 会整体替换旧契约。

```http
PUT http://localhost:3001/v1/generations/team-a-billing
...
如果你直接嵌入 Java Generation SDK，注册入口只有一个：`sdk.register(GenUIGeneration)`。扩展契约就是上面 `generate-component-spec` 导出的那段 JSON（也可以来自配置中心、文件或数据库），用你自己的 JSON 库反序列化成 `GenUIGeneration` 后直接注册，不必在 Java 里手搓嵌套 `Map`：
...
// billing-extension.json 即上面 generate-component-spec 的导出产物
String extensionJson = Files.readString(Path.of("billing-extension.json"));
GenUIGeneration generation = new ObjectMapper().readValue(extensionJson, GenUIGeneration.class);
...
把 tool 放进 Generation Extension：
...
也可以只在单次请求里传 Request Overlay。Overlay 不会持久化，适合“这一次生成才有”的动态 tool：
...
```json
{
  ...
  "tools": [
    {
      "name": "queryCurrentUserBill",
      "description": "查询当前登录用户的账单。",
      "inputSchema": { "type": "object", "properties": {} },
      "annotations": { "readOnlyHint": true }
    }
  ],
  "extraRules": ["当前用户账单只能使用 queryCurrentUserBill。"]
}
```
...
- 前端 Zod props 顺序和后端 `signature` / `propsSchema.properties` 顺序一致。
