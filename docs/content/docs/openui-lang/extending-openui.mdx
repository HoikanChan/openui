...
后端契约不用手写——前端把组件库（`createLibrary` / `library.extend` 的结果）定义好之后，用 CLI 直接从库定义导出，组件名和字段顺序天然和前端一致：

```bash
openui generate-component-spec ./src/billing-library.ts \
  --generation-id team-a-billing \
  --out billing-extension.json
```

导出的是一个**可直接注册的 `GenUIGeneration` 形 JSON**：顶层带 `generationId` / `version`，`components` 用 `propsSchema`（JSON Schema）描述，且只含你扩展的组件，不含 base 内置件：

```json
{
  "generationId": "team-a-billing",
  "version": "1.0.0",
  "components": {
    "BillingCard": {
...
当前 GenUI Service 的 REST 参考实现使用 `/v1/generations` 管理扩展。`generationId` 用来隔离不同业务扩展，重复注册同一个 `generationId` 会整体替换旧契约。
...
1. 服务启动时注册每个业务的 `generationId`。
2. 每次生成时传 `generationId`、`dataModel`、Request Overlay。
3. 后端 assemble system prompt，调用 LLM。
4. 流式返回 `openui-lang`。
5. 前端按同一个 `generationId` 选择匹配的扩展 library，并传入 `toolProvider`。
...
- `generationId` 和前端选择的扩展 library 一一对应。
...
确认服务是否重新注册了对应 `generationId`，请求是否传了正确的 `generationId`，以及前端是否切换到了匹配的扩展 library。
