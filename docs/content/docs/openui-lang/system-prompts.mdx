...
For backends that need dynamic prompts - different tools, preambles, or feature flags per request - use `generatePrompt` from `@cloudsop/openui-lang-core`. This has no React dependency, so it works in any Node/Edge/serverless backend.
