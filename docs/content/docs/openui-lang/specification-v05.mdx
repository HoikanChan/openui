...
These are available in every prompt configuration:

| Function                                  | Signature                                         |
| ----------------------------------------- | ------------------------------------------------- |
| `@Each`                                   | `@Each(array, "item", template)`                  |
| `@Switch`                                 | `@Switch(value, cases, default?) → result`        |
| `@Render`                                 | `@Render("v", expr)`                              |
| `@Render` with full row binding           | `@Render("v", "row", expr)`                       |

`@Render` is intended for prop values where a component expects a render function. Outside a prop context it evaluates to a deferred node and renders as `null`.

### Data builtins

These are available when expression support is enabled:
...
### Iteration and deferred templates

- `@Each(array, varName, template)` - render template for each element. Loop variable is only available inline.
- `@Render("v", expr)` / `@Render("v", "row", expr)` - defer template evaluation until the consuming component injects runtime values.
